...
본 문서와 Usage Guide의 YAML, 코드, schema, command는 재현 가능한 시작점을 제공하기 위한 참고 예시입니다. 동일하게 복사할 필요는 없으며, 선택한 시나리오와 설계에 맞게 다른 구조나 구현을 사용해도 됩니다. 단, 최소 요구사항, 보안 원칙, 재현 가능성, 검증 결과 제출은 반드시 충족해야 합니다.
...
- 제공된 watsonx.data Milvus database를 연결한 wxO Knowledge Base RAG
- Python tool 2개 이상 구현
- 핵심 사용자 질문 5개 정의 및 수동 검증
- RAG와 Python tool을 함께 사용하는 복합 질의 1개 이상
- 근거가 부족한 질의 1개에서 추측하지 않고 부족한 정보와 확인 방법 제시
...
4. **근거 부족 질의**: 문서나 tool 결과만으로 답하기 어려운 이유와 추가 확인 방법 제시

추가 조건:

- 5개 질문 중 최소 2개 이상은 watsonx.data Milvus를 연결한 Knowledge Base에서 문서를 검색해야 합니다.
...
watsonx Orchestrate agent 구성은 ADK YAML/CLI 또는 SaaS UI의 no-code 기능 중 편한 방식을 사용할 수 있습니다.
no-code UI로 구성한 경우에도 평가자가 재현할 수 있도록 agent 이름, instructions, collaborator 연결, tool/Knowledge Base 연결, 테스트 절차를 README에 기록하세요.
Python tool 코드, watsonx.data Milvus 적재/검색 흐름, 검증 결과 제출 요구사항은 동일하게 유지됩니다.
...
- Collaborator의 역할이 명확해야 하며, agent의 `description`이 routing에 사용됨을 고려해 작성
...
본 과제에서 말하는 wxO Knowledge Base는 wxO가 문서를 직접 업로드받아 자체 built-in index를 만드는 방식이 아니라, IBM이 제공하는 **watsonx.data Milvus database를 연결하는 방식**을 의미합니다.

이렇게 분리하는 이유는 후보자가 문서 전처리, chunking, embedding 모델, Milvus collection schema, metadata를 직접 설계하고 설명할 수 있게 하기 위함입니다. wxO built-in document upload Knowledge Base는 전처리와 embedding/index 설정을 세밀하게 통제하기 어렵기 때문에, 최종 제출의 필수 RAG 요건으로 권장하지 않습니다.

- 수집한 문서 8-12개를 전처리하고, 안내된 방식에 따라 지원자별로 분리 제공되는 watsonx.data Milvus database에 적재하여 wxO에서 Knowledge Base에 연결해 사용합니다.
...
- wxO agent는 Knowledge Base를 통해 Milvus 검색 결과를 받아 답변에 사용합니다.
...
- 문서 전처리와 chunk 생성 방식
- Milvus collection schema와 wxO Knowledge Base 설정
- embedding 모델 선택 근거
- retrieval query, `top_k`, score 사용 방식
- 검색 결과의 source metadata

Milvus 접속 정보, API key, password 등 민감정보는 repository에 commit하지 마세요. `.env`, credential 파일, token이 포함된 설정 파일을 올리면 GitHub 또는 사내 secret scanning 경고가 발생할 수 있습니다. 필요한 값은 `.env.example` 또는 README의 placeholder로만 설명하세요.
...
- agent YAML 또는 agent import에 필요한 설정 파일. no-code UI로 구성한 경우에는 README나 `docs/`에 재현 가능한 설정 설명과 스크린샷 포함
- Python tool 코드
...
5. no-code UI 사용 시 설정 기록: agent 이름, instructions, collaborator 연결, tool/Knowledge Base 연결, 주요 화면 스크린샷
6. 수동 검증 결과 요약: 핵심 질문별 기대 동작/실제 결과, 실패 또는 한계 1개
7. 알려진 한계와 다음 단계
8. 사용한 외부 자료/AI/code assistant 도구
...
9. 결과 전달 요약: 고객 문제, 구현 결과, 검증 결과, 한계와 다음 액션
...
- 최소 2개 질의에서 watsonx.data Milvus 기반 RAG 답변과 출처가 함께 제시된다.
...
| 영역 | 비중 | 평가 포인트 |
|---|---:|---|
| **기술적 완성도** | 25% | 실제 동작 여부, agent/tool/RAG 아키텍처 합리성, 평가까지 고려한 설계, end-to-end 재현 가능성 |
| **Business Acumen** | 25% | 문제 정의, KPI, 가설, 기대 ROI 제시, 모호한 요구사항을 명시적 가정과 PoC 범위로 좁히는 능력 |
| **발표 & Q&A** | 25% | 시간관리, 고객 커뮤니케이션, 자료 가독성, trade-off 설명, 모르는 것을 인정하는 솔직함, 압박 질문에서 가정을 재검토하는 능력 |
| **학습력** | 25% | 제시된 IBM 제품 이해와 활용, 요구사항에 맞는 기능 선택, 근거 있는 새 스택 시도와 학습 과정 설명 |
...
네. 단, 최종 제출의 필수 RAG 요건은 wxO built-in document upload Knowledge Base가 아니라, 지원자별로 분리 제공되는 watsonx.data Milvus database를 연결한 Knowledge Base입니다.
