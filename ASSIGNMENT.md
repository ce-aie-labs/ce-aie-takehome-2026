...
- watsonx.data Milvus 기반 Knowledge Base를 활용한 RAG
...
아래 항목은 필수가 아닙니다. 시간이 남을 때만 시도하세요.
...
완성도 높은 작은 구현을 선호합니다. 시간이 부족하면 일부를 의도적으로 가지치기하고, 그 이유와 한계를 README에 적어 주세요.
...
무엇을 선택해야 할지 모르겠다면 아래 기준을 참고하세요.

- 금융/보험: 표, 수치, 계산 tool을 함께 다뤄보고 싶을 때
- 제조/에너지 운영 분석: 운영 지표, 안전 기준, 리포트 문서 기반 RAG를 해보고 싶을 때
- 통신 요금제/고객 문의: 요금제 추천, 고객 문의 routing, 정책 검색을 해보고 싶을 때
- 리테일 배송/반품/재고 운영: 재고, 배송, 프로모션처럼 mock tool을 만들기 쉬운 주제를 원할 때
- 공공/보건/복지 민원: 공개 데이터가 많고 출처 인용 중심의 RAG를 안정적으로 만들고 싶을 때
...
- 5개 질문 중 최소 2개 이상은 watsonx.data Milvus 기반 Knowledge Base에서 문서를 검색해야 합니다.
...
본 과제에서 말하는 wxO Knowledge Base는 IBM이 제공하는 **watsonx.data Milvus database**를 활용하는 Knowledge Base를 의미합니다.
...
- 문서 전처리와 chunk 생성 방식
- Milvus collection schema 또는 Knowledge Base 설정
- embedding 모델 선택 근거
...
4. 기술 선택의 근거
   ...
   - 임베딩 모델
   - chunking 전략
   - Milvus-backed Knowledge Base/retrieval 전략
...
후속 면접에서는 과제 결과를 기반으로 PT 및 기술 Q&A를 진행할 예정입니다. 발표 자료 제출은 필수가 아니며, 준비하는 경우 `slide.pdf` 형태로 repository에 포함할 수 있습니다.

PT 진행 시간과 세부 방식은 후속 면접 안내를 우선합니다. 일반적으로는 50분 내외로 진행하며, 과제 결과 발표 15분과 질의응답 35분을 기준으로 준비하면 됩니다. 발표 자료를 준비하는 경우 제목 포함 5장 이내를 권장합니다.
...
- 최소 2개 질의에서 RAG 답변과 출처가 함께 제시된다.
...
네. 여기서 말하는 Knowledge Base는 지원자별로 분리 제공되는 watsonx.data Milvus database를 활용하는 Knowledge Base를 의미합니다. 안내된 Milvus를 Knowledge Base에 연결해 RAG에 사용하면 됩니다.
...
기능을 많이 나열하기보다, 고객 문제 -> 설계 선택 -> 구현 결과 -> 검증 결과 -> 실패와 다음 액션의 흐름으로 설명할 수 있게 준비하세요. 별도 발표 자료 제출은 필수가 아니며, 후속 PT 또는 기술 Q&A는 제출한 repository, demo, validation 문서, Google Form의 PT 요약을 기반으로 진행합니다.
