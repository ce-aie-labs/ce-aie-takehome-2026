...
`<SHARED_INSTANCE_URL>`과 `<SHARED_API_KEY>`는 wxO SaaS 화면에서 확인하거나 생성해야 합니다.
이 저장소에 포함된 [videos/03_getwxoapikey.mp4](videos/03_getwxoapikey.mp4)에서 instance URL 확인과 API key 생성 흐름을 볼 수 있습니다.
실제 URL/API key는 repository에 commit하지 말고 로컬 shell 또는 `.env`에만 보관하세요.
...
```dotenv
# watsonx Orchestrate
# See videos/03_getwxoapikey.mp4 for how to find the instance URL and create the API key.
WO_ENV_NAME=my-env
