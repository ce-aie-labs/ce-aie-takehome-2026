...
[ASSIGNMENT.md](ASSIGNMENT.md), watsonx.data Milvus 적재 예시는 [MILVUS_USAGE.md](MILVUS_USAGE.md),
...
| 제출 방식 | 발표 자료 PDF를 포함한 Private GitHub repository 생성 후 `jihunkeom`, `minnie0531`, `suyeon96`, `kiyeonjeon21` collaborator 초대, Google Form 제출 |
...
- 제공된 watsonx.data Milvus database를 연결한 Knowledge Base 기반 RAG
...
no-code UI로 구성한 경우에도 agent 이름, instructions, collaborator 연결, tool/Knowledge Base 연결, 테스트 절차를 README에 기록해야 합니다.
...
4. 문서를 전처리해 제공된 watsonx.data Milvus database에 적재하고, wxO Knowledge Base가 해당 Milvus collection을 검색하게 설정합니다.
...
- `agents/`: supervisor/collaborator YAML 또는 import 가능한 agent 설정. no-code UI 사용 시 README나 `docs/`에 재현 가능한 설정 설명과 스크린샷 포함
- `tools/python/`: Python tool 코드
- `knowledge_base/` 또는 `rag/`: Milvus Knowledge Base 설정과 RAG 설명
- `data/sources.md`: 사용한 문서의 URL과 라이선스
- `docs/validation.md` : 핵심 질문 5개 검증 결과
- `demo/slide.pdf`: 후속 PT용 발표 자료
...
- [MILVUS_USAGE.md](MILVUS_USAGE.md): watsonx.data Milvus 적재, 검색, Knowledge Base 연결 가이드
- [WXAI_USAGE.md](WXAI_USAGE.md): watsonx.ai API key/project ID 확인과 SDK 연결 테스트
- [WXO_USAGE.md](WXO_USAGE.md): watsonx Orchestrate agent YAML, Python tool, Milvus Knowledge Base 예시
